...
function signup_user() {
	...
	try {
		...
		if(
			...
			add_user_meta($insert_new_user, '_notification', true);
                        if($track_signup){
                            add_user_meta($insert_new_user, '_track_signup', $track_signup);                            
                        }
			...
			if(get_class(wp_signon( $creds )) == "WP_User"){
				...
				$content .= "<h3>Hey, Thanks for signing up for Coaches Tribune!</h3>";
				$content .= "<p>We're a community for coaches to share tips.<br/>";
				...
				if(isset($new_user["subscribe"]) && $new_user["subscribe"] == "on"){
					$content .= "<p>You're now on the list to receive the weeks best tips and a great deal now and then.</p>";
				}
                                $content_link = false;
                                $content .= '<p>';
                                ...
                                $content .= '</p>';
				$content .= "</div>";
                                $subject = 'Thanks for signing up for Coaches Tribune' ;
          ...
          $msg .= '<div class="modal-content">';
              $msg .= '<div class="modal-body">';
                  ...
                  $msg .= '<div class="login-form-wrapper">';
                    $msg .= '<h2 class="title">Please login to continue!</h2>';
                        ...
                        $msg .= '<div class="form-group">';
                            $msg .= '<div class="row">';
                                $msg .= '<div class="col-xs-12 col-sm-6">';
                                    $msg .= '<button id="login_btn" class="btn btn-lg btn-turquoise btn-block">Log in&nbsp;<i class="fa processing-icon hide"></i></button>';
                                $msg .= '</div>';
                                $msg .= '<div class="col-xs-12 col-sm-6">';
                                    $msg .= '<a href="' . home_url() . '/register" class="btn btn-lg btn-link btn-block signup_modal">or <strong>Join</strong>&nbsp;<i class="fa processing-icon hide"></i></a>';
                                $msg .= '</div>'; 
                                $msg .= '<div class="col-xs-12 col-sm-6">';
                        ...
                        $msg .= '<div class="form-group social-logins">';
                            ...
                            $msg .= '<div class="row">';
                                $msg .= '<div class="col-xs-12 col-sm-6 no-right-padding">';
                                    $msg .= '<a class="fb-login" data-provider="Facebook" title="Connect with Facebook" rel="nofollow" href="' . home_url() . '/admin4214?action=wordpress_social_authenticate&mode=login&provider=Facebook&redirect_to=' . urlencode($current_url) . '"><span class="flaticon flaticon-facebook-logo-button"></span>&nbsp;&nbsp;Log in with Facebook</a>';
                                $msg .= '</div>';   
                                $msg .= '<div class="col-xs-12 col-sm-6">';
                                    $msg .= '<a class="tw-login" data-provider="Twitter" title="Connect with Twitter" rel="nofollow" href="' . home_url() . '/admin4214?action=wordpress_social_authenticate&mode=login&provider=Twitter&redirect_to=' . urlencode($current_url) . '"><span class="flaticon flaticon-twitter-logo-button"></span>&nbsp;&nbsp;Log in with Twitter</a>';
                                $msg .= '</div>';                     
                            $msg .= '</div>';
                        $msg .= '</div>';
                    $msg .= '</form>';
          ...
          $msg .= '<div class="modal-content">';
              $msg .= '<div class="modal-body">';
                  ...
                  $msg .= '<div class="profile-link-wrapper">';
                      $msg .= '<ul>';
                         $msg .= '<li><a href="' . home_url('upload') . '"><i class="flaticon flaticon-directions-signs-outlines"></i> Create and Share a Coaching Tip</a></li>';
                         $msg .= '<li><a href="' . home_url('author/' . $current_user->user_login) . '"><i class="flaticon flaticon-folded-map"></i> Set up profile / my tips page</a></li>';
                         $msg .= '<li><a href="' . home_url('trending') . '"><i class="flaticon flaticon-rocket-icon"></i> View Trending Coaching Tips</a></li>';
                      $msg .= '</ul>';
                  $msg .= '</div>';
              $msg .= '</div>';
