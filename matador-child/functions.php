...
function enqueue_theme_styles_scripts()
{
    ...
    wp_register_script('custom-js', get_stylesheet_directory_uri() . '/js/custom.js', array('jquery'), '1.0.5');
    wp_enqueue_script('custom-js');

    wp_register_style('custom-css', get_stylesheet_directory_uri() . '/custom.css', null, '1.0.7');
    wp_enqueue_style('custom-css');

    wp_register_style('custom-responsive', get_stylesheet_directory_uri() . '/custom-responsive.css');
    ...
    wp_register_script('validate-js', get_stylesheet_directory_uri() . '/inc/validate/validate.js', array('jquery'), '1.0.4');
    wp_enqueue_script('validate-js');
    wp_register_script('login-js', get_stylesheet_directory_uri() . '/inc/frontend/login/js/login.js', array('jquery'), '1.0.2');
    ...
    if (is_page_template('upload.php') || is_post_flow('flow_creation')):
        ...
        wp_register_script('upload-js', get_stylesheet_directory_uri() . '/inc/frontend/upload/upload-jrrny.js', array('jquery'), '1.1.0');
    ...
    if (is_author()):
        wp_register_script('author-js', get_stylesheet_directory_uri() . '/inc/frontend/profile/js/author.js', array('jquery'), '1.0.1');
    ...
    if (is_single()):
        if (isset($_GET["action"]) && $_GET["action"] == "edit"):
            ...
            wp_register_script('edit-single-js', get_stylesheet_directory_uri() . '/inc/frontend/single/js/edit-single.js', array('jquery'), '1.0.5');
        ...
        wp_register_script('vjs-js', get_stylesheet_directory_uri() . '/inc/video-js/video.js', array('jquery'), '5.12.3');
        wp_enqueue_script('vjs-js');

        wp_register_style('vjs-css', get_stylesheet_directory_uri() . '/inc/video-js/video-js.css', null, '5.12.3');
        wp_enqueue_style('vjs-css');
    endif;
    if (is_page('map') || is_single()):
        ...
        wp_register_script('gmaps', 'https://maps.googleapis.com/maps/api/js?key=' . MAPS_API_KEY . '&sensor=false&libraries=places&region=US&language=en');
...
{
    if (!wp_next_scheduled('get_trending')) {
        wp_schedule_event(strtotime('01-01-2016 04:00'), 'daily', 'get_trending_event');
    }
}
...
function send_notification()
{
    ...
    foreach ($ret as $obj) {
        ...
        $content .= "<p>";
        $content .= "You haven't posted a tip yet! Log in and tell the world about your great tip.";
        $content .= "</p>";
        $content .= "</div>";
        $from = "Coachestribune.com <admin@coachestribune.com>";
        $headers = "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html; charset=utf-8" . "\r\n";
        $headers .= "From: $from\r\n";
        $headers .= 'Reply-To: Coachestribune.com <admin@coachestribune.com>\r\n';
...
function subscribe()
{
    if (isset($_POST["event"]) && $_POST["event"] == "subscribe"):
        if (isset($_POST["email"]) && $_POST["email"] !== ""):
            ...
            $content .= "<h3>Hey" . (is_user_logged_in() ? ' ' . $current_user->user_firstname : '') . ", Thanks for signing up for Coaches Tribune!</h3>";
...
add_action('admin_enqueue_scripts', 'plc_h_register_admin_assets');
add_action('wp_enqueue_scripts', 'plc_h_register_frontend_assets');
...
function plc_h_register_admin_assets()
{
...
function plc_h_register_frontend_assets()
{
    wp_enqueue_script('plc_js', get_stylesheet_directory_uri() . '/assets/js/frontend.js', array('jquery'), '1.0.2', true);
...
add_action('init', 'create_featured_destination_type');

function create_featured_destination_type()
{
    register_post_type('featured_destination', array(
        'labels' => array(
            'name' => __('Collections'),
            'singular_name' => __('Featured destination')
        ),
        'public' => true,
        'has_archive' => true,
...
add_action('init', 'create_sponsored_post_type');

function create_sponsored_post_type()
{
...
function plc_access()
{
    if (isset($_POST['password']) && $_POST['password'] === 'betauser') {
        $_SESSION['betauser'] = TRUE;
    }

    if (!isset($_SESSION['betauser']) && !$_SESSION['betauser']) {
        wp_register_style('access-css', get_stylesheet_directory_uri() . '/assets/css/access.css');
        wp_enqueue_style('access-css');

        ob_start();
        get_template_part("inc/frontend/access");
        get_footer();
        $access = ob_get_clean();
        echo $access;
        die();
...
add_filter('wp_mail_from', 'plc_wp_mail_from');

function plc_wp_mail_from($content_type)
{
    return 'admin@coachestribune.com';
}

add_filter('wp_mail_from_name', 'plc_wp_mail_from_name');

function plc_wp_mail_from_name($name)
{
    return 'Coachestribune.com';
}

function filter_email_change_email( $email_change_email, $user, $userdata ) {

    $email_change_email['message'] = str_replace( 'All at ', '', $email_change_email['message'] );
...
include "inc/contest/core.php";
include "inc/amazon/amazon.php";
include "inc/youtube/youtube.php";
include "inc/categoryTags/categoryTags.php";
