...
if (is_user_logged_in()) {
    ...
    if (isset($_FILES["file"]) && $_FILES["file"]["size"] > 0
        ...
        $target_dir = $wp_upload_dir['basedir'] . "/video/" . trim($current_user->user_login) . "/";
        ...
        if (move_uploaded_file($file_tmp_name, $target_file)) {
            $target_url = $wp_upload_dir['baseurl'] . "/video/" . trim($current_user->user_login) . "/" . $file_name;
            $post_mime_type = wp_check_filetype($target_file);
            $attachment = array(
