...
function ts_shortcodes_range ( $range, $all = true, $default = false, $range_start = 1 , $step = 1) {
	if($all === true) {
		$numbers['-1'] = __('All', 'ThemeStockyard');
	}
	
	if(strtolower($all) == 'unlimited' ) {
		$numbers['0'] = __('Unlimited', 'ThemeStockyard');
		$default = false;
	}

	if($default) {
		$numbers[''] = __('Default', 'ThemeStockyard');
	}
...
function ts_shortcodes_categories ( $taxonomy = 'category', $empty_choice = false ) {
	if($empty_choice == true) {
		$post_categories[''] = __('Default', 'ThemeStockyard');
	}
...
$ts_shortcodes['blog'] = array(
	...
	'params' => array(
		...
		'infinite_scroll_limit' => array(
			'type' => 'select',
			'label' => __( 'Infinite Scroll Limit', 'ThemeStockyard'),
			'desc' => __('Maximum number of times the Infinite Scroll should run - only works with "load more on scroll".', 'ThemeStockyard'),
			'options' => ts_shortcodes_range( 30, 'unlimited'),
			'std' => 0
		),
		'infinite_scroll_button_text' => array(
			...
			'std' => ts_option_vs_default('infinite_scroll_button_text', 'Load more posts')
		),
		'cat' => array(
...
$ts_shortcodes['blog_banner'] = array(
	...
	'shortcode' => '[blog_banner columns="{{columns}}" limit="{{posts_per_page}}" fullwidth="{{fullwidth}}" infinite_scroll="{{infinite_scroll}}" infinite_scroll_limit="{{infinite_scroll_limit}}" infinite_scroll_button_text="{{infinite_scroll_button_text}}" cat="{{cat}}" include="{{include}}" exclude="{{exclude}}" exclude_previous_posts="{{exclude_previous_posts}}" exclude_these_later="{{exclude_these_later}}" excerpt_length="{{excerpt_length}}" show_pagination="{{show_pagination}}" ignore_sticky_posts="{{ignore_sticky_posts}}"][/blog_banner]',
