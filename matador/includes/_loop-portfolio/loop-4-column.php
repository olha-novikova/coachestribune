...
?>
                <div class="portfolio-entries-wrap not-masonry-entries-wrap <?php echo esc_attr($edge_class);?>">
                    ...
                    <div class="portfolio-loop-4-column hfeed portfolio-entries entries loop <?php echo esc_attr($entries_class);?> clearfix">
                        <?php
                        if($ts_query->have_posts()) : 
                            $i = 1;
                            while($ts_query->have_posts()) : $ts_query->the_post();
                        ?>                        
                        <div id="post-<?php echo esc_attr($ts_query->post->ID);?>" class="entry hentry ts-boxed-one-fourth <?php echo esc_attr(ts_get_the_category('portfolio-category', 'filter-slugs', ' '));?>">
                            <div class="post-content">
                                <?php
                                echo ts_get_featured_media(array('media_width'=>360,'media_height'=>300,'allow_videos'=>'no'));
                                ?>  
